      ...
      switch(keys[key]) {
        case "required":
          if (this.doc.forms[element][index]["required"] === false && input.value !== "") {
            ...
            input.required = true; 
           if(messageCheck === false) message = "Please fill in this field";
          }
          break;
        case "minLength":
        case "maxLength":
        ...
        case "checkWriting":
          if (this.doc.forms[element][index]["checkWriting"] === true && inputType === "password"){
            ...
            if (!filter.test(input.value)) {
              if(messageCheck === false) message = "Password not written in proper way, please use following notation: Minimum eight characters, at least one uppercase letter, one lowercase letter, one number and one special character";
            } 
          }
          break;
        case "compareWith":
          if (this.doc.forms[element][index]["compareWith"].length > 0 && inputType === "password" && input.value !== null){
        ...
        case "minVal":
        case "maxVal":
          if (inputType === "number" && this.doc.forms[element][index]["minVal"] === true && input.value !== null){
            if(inputType === "number") input.min = this.doc.forms[element][index]["minVal"];
            if(messageCheck === false && input.value < this.doc.forms[element][index]["maxVal"]) message = `Number mustn\'t be under ${this.doc.forms[element][index]["minVal"]}`;
          } else if (inputType === "number" && this.doc.forms[element][index]["maxVal"] === true && input.value !== null) {
            if(inputType === "number") input.max = this.doc.forms[element][index]["maxVal"];
            if(messageCheck === false && input.value > this.doc.forms[element][index]["maxVal"]) message = `Number mustn\'t be above ${this.doc.forms[element][index]["maxVal"]}`;
          }
          break;
      }
